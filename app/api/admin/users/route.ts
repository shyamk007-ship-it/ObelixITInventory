...
export async function POST(request: Request) {
  ...
  if (isOwnerEmail(email)) {
    return NextResponse.json({ error: "Owner account is managed automatically." }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to finalize user creation." },
      { status: 500 }
    );
  }
